...
/**
 * Чистый циферблат: круг/фон, деления, цифры, стрелки и перетаскивание стрелок.
 * Без текста времени и переключателей — переиспользуемое ядро.
 *
 * @param time текущее время.
 * @param onTimeChange вызывается при изменении времени перетаскиванием стрелки.
 * @param modifier модификатор контейнера циферблата.
 * @param config оформление: цвета, фон, стрелки, цифры, размер.
 * @param snapTo5Minutes привязывать минуты к ближайшим 5 при перетаскивании.
 */
@Composable
fun AnalogClockDial(
  time: LocalTime,
  ...
  fun minuteAt(position: Offset, size: Size): Int =
    ClockMath.minuteFromAngle(angleAt(position, size), snapTo5Minutes)
  ...
  Box(modifier = modifier.size(config.radius * 2)) {
      Canvas(
        modifier = Modifier
          .fillMaxSize()
          .pointerInput(updateTime, minutes, snapTo5Minutes) {
...
/**
 * Удобная обёртка над [AnalogClockDial]: добавляет опциональные текст времени
 * сверху и переключатель привязки к 5 минутам снизу.
 *
 * @param time текущее время.
 * @param onTimeChange вызывается при изменении времени.
 * @param modifier модификатор колонки.
 * @param config оформление циферблата.
 * @param showTimeText показывать ли текст времени сверху.
...
/**
 * Диалог выбора времени с аналоговым циферблатом и кнопками подтверждения/отмены.
 *
 * @param initialTime время, показываемое при открытии.
 * @param onTimeSelected вызывается с выбранным временем при подтверждении.
